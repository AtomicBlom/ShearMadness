...
public class InfiltratorModelMaker extends DefaultModelMaker
{
    ...
    {
        final QuadrupedModel<SheepEntity> model  = super.createModel(new ItemStack(Items.WHITE_WOOL), entity, spriteMap);
        ...
        final TextureAtlasSprite chickenWings = atlasTexture.getSprite(Reference.Textures.CHICKEN_WINGLETS);
        ...
        rightForeLeg.addTexturedQuads(new Matrix4f(legTransform), new Matrix3f(), createLeg(chickenSprite));
        model.legFrontRight.cubeList.add(rightForeLeg);

        EntityMesh leftForeLeg = new EntityMesh();
        leftForeLeg.addTexturedQuads(new Matrix4f(legTransform), new Matrix3f(), createLeg(chickenSprite));
        model.legFrontLeft.cubeList.add(leftForeLeg);

        EntityMesh rightRearLeg = new EntityMesh();
        rightRearLeg.addTexturedQuads(new Matrix4f(legTransform), new Matrix3f(), createLeg(chickenSprite));
        model.legBackRight.cubeList.add(rightRearLeg);

        EntityMesh leftRearLeg = new EntityMesh();
        leftRearLeg.addTexturedQuads(new Matrix4f(legTransform), new Matrix3f(), createLeg(chickenSprite));
        model.legBackLeft.cubeList.add(leftRearLeg);

        return model;
    }

    private TexturedQuad[] createLeg(TextureAtlasSprite chickenSprite) {
        TexturedQuad[] texturedQuads;
